...
The first one, IPFS, can (?) produce a permanent, robust, cheap, indipendent, disk, for every one; it's not the first filesystem distributed on top of the pre-existing "[The Internet for Everyone](http://www.ietf.org/rfc/rfc3271.txt)" but this is the first that exposes file hashes to the user interface, it simply looks better (than plan9, gfs, etc). 
