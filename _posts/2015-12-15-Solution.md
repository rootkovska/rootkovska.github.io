...
This time we are doing it not at file level, but at block level: no matter how much WIPO uniformed the Intellectual Protection practices around the world, a block, by itself, content nothing. And all of them, a content, are everywhere in the world: costly to track, fucking expensive to persecute.
...
Note to the italian failing chapter:
...
* [Gino Paoli](https://en.wikipedia.org/wiki/Gino_Paoli#Legal_problems), 2013-2015 President of the italian collecting society SIAE, proved him wrong.

After that, there's steganography (ex: arts carried by bitcoin blocks). It would further slow down our messaging, but it is pretty interesting stuff to add on top to distribution and cryptography. Just remember, just in case [the current EU law](http://eur-lex.europa.eu/legal-content/EN/TXT/?uri=uriserv:OJ.L_.2014.084.01.0072.01.ENG) will fail, [as the previous one did](http://eur-lex.europa.eu/legal-content/IT/TXT/?uri=CELEX:32001L0029), and so on ... 

# Conclusion

This stuff might be already operational in the big datacenters. Personally I've spotted plenty time-screws in the last 8 years that made me think to off side track adjustments. For a few years I haven't been able to find webpages with a date on page. Public events just disappears from my device. And so on.
By using multiple concurrent, nested, blockchains, nobody can manipulate us all in the same way.
