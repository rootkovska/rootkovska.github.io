...
5. Every single person can spawn its own "constitutional work" using this tool instead of the legacy way I produced. By using the same minimal tool, any future integration attempt with other individual or group legal work can be easily merged providing a semantic map. It's the beauty of the standards. Previous art: the TCP/IP (stack of) protocol(s); as well as part of the process following companies acquisition. 
...
2. There are plenty empirical evidences that show general asyncronicity among human beings: drugs, rapid crossing of timezones (ex: jetlag), prolongued isolation states, etc. Once given - again, in my case by trust not by knowledge - Hawking's words for good, the financials theories can be applied to individuals intead of groups of individuals (ex: family, nation, etc). 
...
1. At this point in time the Ethereum Foundation bought bitcoins to sell ethers. The Ethereum blockchain uses "gas" (cryptocurrency) to account its operations. The website is pretty childish (it looks like targetting kids!) but the concept seems valid both for information work accounted in the time domain ( euro = (ether * t) / tau ) and atoms work accounted in product units ( euro = (ether * shoe) / tau  ). 
...
3. Someone says that the evaluation of tau could lead to the end of privacy in a similar fashion the attitude tests did; indeed it is possible to self-asses tau using the proper Turing-test-like procedure. Other apocaliptic theories may spawn (but we are all used to those): price discrimination, capitalism's destruction, etc. Fearmongers ... 

4. The UN Declaration of Human Rights (or any other local equivalent) can be used to re-offset economics so that "needs" are always satisfied (ex: 50 liters of water, 2500 kcal per day). The entire system can be simulated in public. 
