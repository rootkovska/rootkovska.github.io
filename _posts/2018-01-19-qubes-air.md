...
Amazon EC2, Microsoft Azure, Google Compute Engine, or even a decentralized
computing network, such as [Golem]. This is illustrated (in a very simplified
way) in the diagram below:
...
running either on the user's local machine, or... in the cloud, as described
above. Again, there will be pros and cons to both approaches, but Qubes should
support both -- and mostly seamlessly from the user's and admin's points of view
(as well the Qubes service developer's point of view!).
